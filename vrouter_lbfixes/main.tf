...
resource "opennebula_virtual_router_instance" "router" {
  ...
  virtual_router_id = opennebula_virtual_router.router[count.index].id
}
...
resource "opennebula_virtual_machine" "vm" {
  ...
  count       = 2
  name        = "vm-${count.index}"
  template_id = opennebula_template.backend.id

  nic {
    network_id = data.opennebula_virtual_network.private.id
    gateway    = opennebula_virtual_router_nic.eth1[count.index].ip
    dns        = opennebula_virtual_router_nic.eth1[count.index].ip
  }
}
