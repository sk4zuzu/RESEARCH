...
resource "libvirt_pool" "s2" {
  name = "s2"
  type = "dir"
  path = var.storage.directory
}

resource "libvirt_volume" "s2" {
  name   = "s2"
  pool   = libvirt_pool.s2.name
  format = "qcow2"
  source = var.storage.artifact
}

resource "libvirt_cloudinit_disk" "s2" {
  name = "s2.iso"
  pool = libvirt_pool.s2.name
...
resource "libvirt_domain" "s2" {
  name = "s2"

  cloudinit = libvirt_cloudinit_disk.s2.id
  ...
  disk {
    volume_id = libvirt_volume.s2.id
  }
